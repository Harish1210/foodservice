...
export default async function HomePage({ searchParams }: { searchParams: Promise<{ vendor?: string }> }) {
  ...
  return (
    <div className="min-h-screen bg-[#FFF8F0]">
      <Navbar />
      {/* Vendor hero banner */}
      <div className="relative overflow-hidden" style={{ background: "linear-gradient(135deg, #0D0500 0%, #3D1200 50%, #7C2D12 100%)" }}>
        ...
        <div className="relative z-10 max-w-3xl mx-auto px-4 py-12 text-center">
          ...
          {vendor && (
            <div className="inline-flex items-center gap-2 mb-5 bg-white/10 border border-white/20 px-4 py-1.5 rounded-full">
              <span className={`w-2 h-2 rounded-full ${vendor.isOpen ? "bg-green-400 shadow-sm shadow-green-400" : "bg-gray-400"}`} />
              <span className={`text-sm font-semibold ${vendor.isOpen ? "text-green-300" : "text-gray-400"}`}>
          ...
          <div className="flex flex-wrap justify-center gap-2 text-xs">
            {[
              ...
              <span key={p.label} className="bg-[#FF6B00]/20 border border-[#FF6B00]/40 text-[#FFB87A] px-3 py-1.5 rounded-full font-medium">
                {p.icon} {p.label}
              </span>
            ))}
          </div>
